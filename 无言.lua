...
if treeClass=="LoveCave" and axestr.Damage<10000000 then
    return notify("MGT","你没有合适的斧头去砍这棵树")  
  end
...
delete=function(V, P)
	...
	local getTool = function()
	  ...
	  local attachToPlayer = function(player, inLoop, kick)
		...
		if not getTool() then return Notify("MGT", "请你拿上斧头", 3) end
...
function SaveSlot()
 ...
 function dupeitemstoplayer(playername)
	...
	if not game:GetService("ReplicatedStorage").Interaction.ClientIsWhitelisted:InvokeServer(autoDupeTo) then
		notify("MGT", "这个玩家需要给你MGT名单",3)
		return
	end
	if not lp.WhitelistFolder:FindFirstChild(tostring(autoDupeTo)) then
		notify("MGT", "正在给 "..tostring(autoDupeTo).."MGT名单",3)
...
local win = bailib:new("MGT--伐木大亨4")
...
Section:Button("如何食用加载复制斧头", function()
	notlfy("MGT", "默认死亡时间为3.1秒，如果卡在加载界面就增加时间,如果斧头被保存了就减少时间,加减推荐是在3.1的基础上来,推荐加减时间为0.1秒", "好")
end)
Section:Textbox("死亡后多长时间加载",'Toggleflag',"输入数字",function(txt)
	bai.loaddupeaxewaittime=txt
end)
Section:Button("获取当前死亡后加载时间", function()
	notlfy("MGT", " 时间为"..bai.loaddupeaxewaittime.."秒", "好")
end)
Section:Button("保存当前死亡后加载的时间", function()
	writefile("MGT斧头复制时间.txt", bai.loaddupeaxewaittime)
end)
Section:Button("读取保存后加载的时间", function()
	local dupeaxetime = isfile("MGT斧头复制时间.txt")

		if dupeaxetime==true then
			local contents = readfile("MGT斧头复制时间.txt")
			bai.loaddupeaxewaittime=contents 
		else
			bailib:Notification("MGT", "你还没有保存或是把文件删除了", "好")
		end
end)
...
spawn (function()
	...
	if not ReplicatedStorage.LoadSaveRequests.ClientMayLoad:InvokeServer(ren) then
				notify("MGT", "请等待，能够加载的时候自动会加载的", 3)
				repeat
					wait(5)
				until ReplicatedStorage.LoadSaveRequests.ClientMayLoad:InvokeServer(ren)
			end;

					notify("MGT", "开始自我复制......", 10)
						notify("MGT ", "如果卡住请大退游戏", 10)
...
Section5:Dropdown("选择树",'Dropdown',{'泰坦木',"普通树","沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "MGT桦木", "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树",'粉木','煤炭幻影','黑木','粉色巧克力树','岩浆椰子树','幽灵木','大幽灵木','蓝光椰子树','爬行者树','幻影树','粉色大幽灵木','蓝色大理石幻影','人脸木','太空椰子木','糖果木'},function(b)
	if b=='普通树'then
...
Sectionhuanjin:Toggle("终日MGT天",'Toggleflag',false, function(state)
	if state then 
		bai.awaysday=true
		while task.wait() do
...
Sectionzidongjianzao:Dropdown("选择你想要填充的木头",'Dropdown',{"普通树","沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "MGT桦木", "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树",'粉木','煤炭幻影','黑木','粉色巧克力树','岩浆椰子树','幽灵木','大幽灵木','蓝光椰子树','爬行者树','幻影树','粉色大幽灵木','蓝色大理石幻影','人脸木','太空椰子木','糖果木'},function(b)
	if b=='普通树'then
